...
function coreInstructions(ctx: ToolContext, platform: NodeJS.Platform): string {
  ...
  if (windows && (ctx.caps.screen || ctx.caps.control || ctx.caps.clipboardRead || ctx.caps.clipboardWrite)) {
    lines.push(
      ...
      `Seeing and controlling the Windows desktop lives in a separate connector, "${surfaceDefinition('desktop').suggestedConnectorName}".`,
...
function desktopInstructions(ctx: ToolContext): string {
  ...
  lines.push(
    '',
    `Files, patches and commands live in a separate connector, "${surfaceDefinition('core').suggestedConnectorName}".`,
