 ...
 *   · /hello and /pair/challenge expose no authority; /pair mints a bearer token only after
 *     a single-use HMAC challenge proves possession of this install's materialized companion proof
...
export async function unpair(): Promise<void> {
  ...
  // Revocation rotates the install proof too. If materializing the replacement resource fails,
  // the bearer is already revoked and reconnect stays fail closed until the app repairs it.
  rotateCompanionPairingProof();
...
async function handle(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  if (route === '/pair/challenge' && req.method === 'POST') {
    ...
    return json(res, 200, { challenge: issueCompanionPairingChallenge() }, origin);
  }
  ...
  if (route === '/pair' && req.method === 'POST') {
    ...
    const proof = verifyCompanionPairingResponse(record['challenge'], record['response']);
    if (proof === 'invalid_challenge') {
      return json(res, 409, { error: 'invalid_pairing_challenge' }, origin);
    }
    if (proof !== 'ok') {
      return json(res, 403, { error: 'invalid_companion_proof' }, origin);
    }
    const reconnect = record['reconnect'] === true;
    ...
    // The HMAC response proves possession of an install-local secret that is generated by this
    // app and copied only into its materialized companion. Origin is deliberately not part of
    // that identity proof: Chrome may omit it, and any HTTP client can spoof a header string.
    // A successful pair still rotates the ordinary bearer token, so each mint supersedes the
    // previous browser credential without turning the long-lived install proof into a bearer.
    const token = randomBytes(32).toString('base64url');
    await setSecret('bridgeToken', token);
    noteBrowserSeen();
    logInfo('bridge: authenticated companion connected and provisioned');
    changed();
  ...
  if (route === '/unpair' && req.method === 'POST') {
    await unpair();
...
export async function stopBridge(): Promise<void> {
  ...
  // A challenge is meaningful only for the currently published listener lifetime.
  clearCompanionPairingChallenges();
  const epoch = ++bridgeLifecycleEpoch;
  const request = enqueueBridgeLifecycle(async () => {
