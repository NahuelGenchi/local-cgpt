 ...
 *
 * Permission disclosures have one presentation-state owner here. Multiple groups may
 * stay open in their bounded scroll pane; a cockpit observer no longer overrides them.
 * Live clocks still tick locally between main-process status reports.
 */

import type { AppApi, SettingsPatch } from '../preload/index.js';
import { requiresApprovedFilesystemRoot } from '../shared/capabilities.js';
import { connectionPresentation } from '../shared/connection-presentation.js';
...
import { renderHomeCockpit } from './cockpit.js';
...
const GROUPS: Group[] = [
  { id: 'read', title: 'Look at files', icon: 'i-eye', blurb: 'Read and search inside the folders you approved.', caps: ['browse', 'search', 'read', 'metadata'] },
  { id: 'write', title: 'Change files', icon: 'i-pencil', blurb: 'Create, edit, move and delete, inside those folders only.', caps: ['create', 'edit', 'move', 'deleteFile'] },
  { id: 'reference', title: 'Read public references', icon: 'i-eye', blurb: 'Read only the public engineering/specification pages reviewed into this app.', caps: ['publicReference'] },
  { id: 'desktop', title: 'See and use the desktop', icon: 'i-monitor', blurb: 'Screenshots, the list of open windows, and the mouse and keyboard.', caps: ['screen', 'control', 'clipboardRead', 'clipboardWrite'] },
  { id: 'run', title: 'Run programs', icon: 'i-terminal', blurb: 'Start commands as you. Autonomous projects add resumable task/process ownership on top of command access.', caps: ['command', 'projectAutonomy'] },
  { id: 'github', title: 'Use GitHub', icon: 'i-out', blurb: 'Publish committed branches and create GitHub issues or pull requests. Ordinary commands stay offline.', caps: ['network'] }
];
let state: AppState | null = null;
/** Guards against saving while we are writing values into the controls. */
let applying = false;
/** Only severity bits, not another copy of sensitive log text. Same 500-row retention as the feed. */
const retainedProblemFlags: boolean[] = [];
let recentProblems = 0;

/**
 * Applies persisted form state without erasing a value the user is currently editing.
 * `state:changed` is primarily a live status push but carries the whole config. Protect
 * only a focused dirty edit; a focused-but-clean control follows persisted state normally.
 */
...
/** Expansion is presentation only. This Set never changes a capability or settings IPC. */
...
function showTab(name: string): void {
  for (const tab of document.querySelectorAll<HTMLElement>('nav button')) tab.classList.toggle('is-sel', tab.dataset.tab === name);
  for (const panel of document.querySelectorAll<HTMLElement>('.panel')) panel.classList.toggle('is-active', panel.dataset.panel === name);
  // Chat costs work to refresh; never reload it while hidden. Hidden feeds have no geometry,
  // so restore their remembered pin only when their panel is actually shown.
...
// ------------------------------------------------------------ permissions
/** Builds each permission header once; expansion and authority remain separate native controls. */
...
/** Tool names come from the authoritative capability mapping rather than duplicated prose. */
...
function buildGroups(): void {
  ...
  // Recording and workers control tool exposure. Keep the sole switches here; Chat settings
  // contains only tuning values, not a second checkbox for the same authority.
...
/** Refreshes counts, tri-state switches and read-only masking without overwriting expansion. */
function paintGroups(): void {
  ...
  for (const group of GROUPS) {
    ...
    root.classList.toggle('is-open', openGroups.has(group.id));
    ...
    root.querySelector<HTMLElement>('.group-count')!.textContent = usable.length === 0
      ? 'off in read-only mode' : on.length === 0 ? 'off' : on.length === group.caps.length
      ? `${on.length} permission${on.length === 1 ? '' : 's'}` : `${on.length} of ${group.caps.length} permissions`;
  ...
  // apply() already protected focused dirty switches. Recopying persisted state here would
  // erase an in-flight user edit when an unrelated stale status push arrives.
  for (const [id, onText] of [['recording', 'session tool exposed'], ['agents', 'agents tool exposed']] as Array<[string, string]>) {
...
/** Published schemas come from the main-process surface report, never checkbox counting. */
function toolsOn(next: AppState): number {
  return next.status.surfaces.filter((surface) => surface.available).reduce((sum, surface) => sum + surface.tools.length, 0);
}

// ------------------------------------------------------------------ save
// Capture each desired snapshot immediately, deriving it from the latest REQUESTED state.
// Serializing only IPC would still let a later click inherit stale readOnly/theme values.
...
function save(over: { readOnly?: boolean; theme?: 'light' | 'dark' } = {}): Promise<void> {
  ...
  const previous: AppState['config'] = requestedSettings ? { ...state.config, ...requestedSettings } : state.config;
  ...
  for (const input of document.querySelectorAll<HTMLInputElement>('[data-cap]')) {
    ...
    // Hidden unsupported Desktop checkboxes are presentation, not user edits. Preserve
    // explicit Windows settings rather than erasing them on every Linux/macOS save.
  ...
  const work = settingsSaveQueue.then(() => saveSnapshot(patch, previous), () => saveSnapshot(patch, previous));
  settingsSaveQueue = work.then(() => undefined, () => undefined);
  return work;
}
async function saveSnapshot(patch: SettingsPatch, previous: AppState['config']): Promise<void> {
  const toolSurfaceChanged = previous.sessions.record !== patch.sessions.record ||
  ...
  const base: SettingsPatch = {
    capabilities: previous.capabilities, readOnly: previous.readOnly, tunnel: previous.tunnel,
    ui: previous.ui, sessions: previous.sessions, compaction: previous.compaction,
    multiAgent: previous.multiAgent, goal: previous.goal
  };
  const next = await run(api.saveSettings(patch, base));
  if (next) {
    apply(next);
    if (previous.multiAgent.enabled && !patch.multiAgent.enabled) {
      // Cached connector schemas can survive revocation; never promise a hot client rewrite.
      toast('Multi-agent off. Reconnect the connector in ChatGPT (then start a new chat) to drop the agents tool.');
    } else if (toolSurfaceChanged) toast('Tools changed. Start a new ChatGPT conversation to guarantee the new tool list is loaded.');
  } else await refresh();
  // A completed older save must not erase the desired state of a newer queued save.
...
const STATUS_TEXT: Record<AppState['status']['state'], string> = {
  ...
  connected: 'Connected', offline: 'No internet', 'auth-failed': 'Sign-in failed', 'tunnel-unavailable': 'Tunnel unavailable'
};
const METHOD_HINT: Record<string, string> = {
  openai: 'ChatGPT reaches this computer through an OpenAI tunnel. Nothing is exposed to the open internet.',
  cloudflared: 'Creates a temporary public https address with Cloudflare. The address changes on every restart.',
...
/** Offline is still running: Disconnect must stop the retrying owned tunnel. */
function isRunning(value: AppState['status']['state']): boolean {
  return value === 'connected' || value === 'offline' || value === 'starting-server' || value === 'connecting-tunnel';
}
/** Same root/credential admission prerequisites as the main-process wizard contract. */
function missingStep(next: AppState): { step: string; text: string } | null {
  const { config } = next;
  if (config.roots.length === 0 && requiresApprovedFilesystemRoot(config)) return { step: 'folder', text: 'Choose a folder to share — step 1.' };
  if (config.tunnel.kind === 'openai') {
    if (!TUNNEL_ID_PATTERN.test(config.tunnel.tunnelId)) return { step: 'tunnel', text: 'Create a tunnel and paste its ID — step 2.' };
    if (!(next.secureStorage?.available ?? true) && !next.hasApiKey) return { step: 'key', text: next.secureStorage?.detail ?? 'Secure credential storage is unavailable.' };
    if (!next.hasApiKey) return { step: 'key', text: 'Add a restricted API key — step 3.' };
  } else if (!next.resolvedBinary && config.tunnel.kind === 'cloudflared') return { step: 'connect', text: 'cloudflared was not found on this computer.' };
  return null;
}
interface RootRenameState {
  targetName: string; targetPath: string; draft: string;
  selectionStart: number | null; selectionEnd: number | null;
  selectionDirection: 'forward' | 'backward' | 'none' | null;
  focused: boolean; committing: boolean;
}
let rootRename: RootRenameState | null = null;
let repaintingRoots = false;
/** Preserve a transient editor only while the exact authoritative root still exists unchanged. */
...
async function commitRootRename(input: HTMLInputElement, rename: RootRenameState): Promise<void> {
  ...
  if (!nextName || nextName === rename.targetName) { cancelRootRename(); return; }
  rename.committing = true;
  input.disabled = true;
  const result = await run(api.renameRoot(rename.targetName, nextName));
  // An intervening authoritative removal/rename cancels this editor; never resurrect it.
  if (rootRename !== rename) return;
  if (result) { rootRename = null; apply(result); return; }
  // Failure is retryable user input, not permission to discard the draft.
...
function rootRow(root: AppState['config']['roots'][number]): HTMLElement {
  ...
  const renameState = rootRename?.targetName === root.name && rootRename.targetPath === root.path ? rootRename : null;
  const name = el('b', '', `/${root.name}`);
  let label: HTMLElement = name;
  if (renameState) {
    const input = document.createElement('input');
    input.className = 'root-rename'; input.value = renameState.draft; input.maxLength = 32; input.disabled = renameState.committing;
    ...
    input.addEventListener('keydown', (event) => {
      if (event.key === 'Enter') { event.preventDefault(); void commitRootRename(input, renameState); }
      else if (event.key === 'Escape') { event.preventDefault(); cancelRootRename(); }
    });
    input.addEventListener('blur', () => {
      // replaceChildren may itself blur a node. A repaint is not user intent to commit.
  ...
  rename.className = 'btn'; rename.type = 'button'; rename.title = `Rename /${root.name}`;
  rename.append(icon('i-pencil'));
  rename.addEventListener('click', () => {
    rootRename = { targetName: root.name, targetPath: root.path, draft: root.name, selectionStart: 0, selectionEnd: root.name.length, selectionDirection: 'none', focused: true, committing: false };
    paintRoots(state?.config.roots ?? []);
  });
  const remove = document.createElement('button');
  remove.className = 'btn'; remove.type = 'button'; remove.title = `Stop sharing /${root.name}`;
  remove.append(icon('i-trash'));
  remove.addEventListener('click', async () => { const result = await run(api.removeRoot(root.name)); if (result) apply(result); });
...
function paintRoots(roots: AppState['config']['roots']): void {
  ...
  if (rootRename && !roots.some((root) => root.name === rootRename!.targetName && root.path === rootRename!.targetPath)) rootRename = null;
  repaintingRoots = true;
  try { $('rootList').replaceChildren(...roots.map(rootRow)); }
  finally { repaintingRoots = false; }
  if (!rootRename?.focused) return;
  const input = document.querySelector<HTMLInputElement>('.root-rename');
  if (!input) return;
  input.focus();
  if (rootRename.selectionStart !== null && rootRename.selectionEnd !== null) input.setSelectionRange(rootRename.selectionStart, rootRename.selectionEnd, rootRename.selectionDirection ?? undefined);
}
...
function apply(next: AppState): void {
  ...
  const connectionView = connectionPresentation(next);
  const dark = config.ui.theme === 'dark';
  document.documentElement.dataset.theme = dark ? 'dark' : 'light';
  $('themeIcon').setAttribute('href', dark ? '#i-sun' : '#i-moon');
  $('themeBtn').title = dark ? 'Switch to light mode' : 'Switch to dark mode';
  const live = $('live');
  live.className = `live${connected && connectionView.tone === 'good' ? ' is-connected' : offline ? ' is-offline' : busy ? ' is-busy' : failed ? ' is-error' : ''}`;
  $('liveState').textContent = connectionView.label;
  const id = config.tunnel.tunnelId;
  $('headerSub').textContent = config.tunnel.kind === 'openai'
    ? TUNNEL_ID_PATTERN.test(id) ? `${id.slice(0, 11)}…${id.slice(-4)}` : 'No tunnel yet'
  ...
  applyChecked($<HTMLInputElement>('homeMaEnabled'), config.multiAgent.enabled, previousState?.config.multiAgent.enabled);
  // Recording is a tool switch, with the same dirty-field guard as other exposure controls.
  applyChecked($<HTMLInputElement>('sessRecord'), config.sessions.record, previousState?.config.sessions.record);
  paintGroups();
  paintRoots(config.roots);
  $('rootsEmpty').hidden = config.roots.length > 0;
  $('setupBadge').hidden = missing === null;
  applyValue($<HTMLSelectElement>('tunnelKind'), config.tunnel.kind, previousState?.config.tunnel.kind);
  ...
  applyValue($<HTMLInputElement>('desktopTunnelId'), config.tunnel.desktopTunnelId, previousState?.config.tunnel.desktopTunnelId);
  ...
  applyChecked($<HTMLInputElement>('minimizeToTray'), config.ui.minimizeToTray, previousState?.config.ui.minimizeToTray);
  applyChecked($<HTMLInputElement>('privacyScreenshots'), config.ui.privacyScreenshots, previousState?.config.ui.privacyScreenshots);
  $('privacyScreenshotsSetting').hidden = !(next.platform?.desktopAutomation ?? true);
  if (next.platform?.family === 'macos') {
    $('backgroundRunningCopy').textContent = 'Leave it running while you use the connector. It stays available from the menu bar and Dock when you close the window.';
    $('minimizeToTrayCopy').textContent = 'Hide the window to the menu bar when closed';
  } else {
    $('backgroundRunningCopy').textContent = 'Leave it running while you use the connector. It stays in the tray when you close the window.';
  ...
  // OpenAI needs one tunnel per surface; manual/Cloudflare publish the entire local origin.
  ...
  $('wizFolders').textContent = config.roots.length === 0 ? 'None yet' : config.roots.map((root) => `/${root.name}`).join('  ');
  ...
  $('wizStatus').textContent = running || failed ? (config.tunnel.kind === 'manual' ? connectionView.detail : status.detail || STATUS_TEXT[status.state]) : '';
  $('chatgptConn').replaceChildren(openai
  ...
  // A discovered connector, a successful tool call, and a healthy optional second connector
  // are different facts. Never use a Core request to mark Desktop setup complete.
  const unverified = status.surfaces.filter((surface) => surface.available && surface.lastRequestAt === null);
  const chatgptNote = $('wizChatgpt');
  chatgptNote.classList.toggle('is-warn', status.lastRequestAt !== null && (status.lastToolCallAt === null || unverified.length > 0));
  chatgptNote.textContent = status.lastRequestAt === null
    ...
    : status.lastToolCallAt === null
      ...
      : unverified.length > 0
        ? `ChatGPT ran a tool ${ago(status.lastToolCallAt)}, but ${unverified.map((surface) => `“${surface.connectorName}”`).join(' and ')} has never been called — create it in ChatGPT to use it.`
        : `ChatGPT ran a tool ${ago(status.lastToolCallAt)} — the whole chain works.`;
  const cards = $('connectorCards');
  // Unfinished optional setup stays visible even after required setup has been completed.
  ...
  // Connect marks local transport readiness; the separate ChatGPT step requires a real
  // request on each required surface. Optional Desktop never blocks core setup completion.
  const requiredUnverified = status.surfaces.some((surface) => surface.available && !surface.optional && surface.lastRequestAt === null);
  if (status.lastRequestAt !== null && !requiredUnverified) done.add('chatgpt');
  // Durable pairing is authorization, not liveness. Required browser features need a check-in
  // during this process; disabled optional browser augmentation must not block setup.
  ...
  $('binaryState').textContent = !needsBinary ? 'Not needed for this method.' : next.resolvedBinary ? `Using ${next.resolvedBinary}` : 'Not found. Install it, or choose the file with Browse.';
  ...
  renderHomeCockpit(next, { setupProblem: missing?.text ?? null, problems: recentProblems });
  applying = false;
}

const SURFACE_STATE_TEXT: Record<SurfaceStatus['state'], string> = { off: 'Not published', starting: 'Connecting…', live: 'Published', error: 'Problem' };
/** Copy only an explicit user-selected value through the existing fixed IPC. */
function copyRow(label: string, value: string, what: string): HTMLElement {
  const field = el('div', 'field');
  const input = document.createElement('input');
  input.type = 'text'; input.readOnly = true; input.spellcheck = false; input.value = value;
  const button = el('button', 'btn btn-solid');
  (button as HTMLButtonElement).type = 'button';
  button.append(icon('i-copy'), document.createTextNode('Copy'));
  button.addEventListener('click', async () => { const copied = await run(api.writeClipboard(value)); if (copied) toast(`${what} copied`); });
  const row = el('div', 'row-inline');
  row.append(input, button);
  field.append(el('label', '', label), row);
  return field;
}
/** Exact connector names/descriptions are copyable because discovery/routing relies on them. */
function connectorCards(next: AppState): HTMLElement[] {
  const { status, config } = next;
  return status.surfaces.filter((surface) => surface.id !== 'desktop' || (next.platform?.desktopAutomation ?? true)).map((surface) => {
    const card = el('div', `connector is-${surface.state}`);
    const head = el('div', 'connector-head');
    const label = config.tunnel.kind === 'manual' && surface.state === 'live'
      ? surface.lastRequestAt === null ? 'Local server ready' : 'Remote connection verified'
      : SURFACE_STATE_TEXT[surface.state];
    head.append(el('h4', '', surface.connectorName), el('span', 'tag', surface.optional ? 'optional' : 'required'), el('span', `pill is-${surface.state}`, label));
    card.append(head, el('p', 'hint', surface.cardSummary));
    if (!surface.available) { card.append(el('p', 'hint', surface.detail)); return card; }
    card.append(copyRow('Name', surface.connectorName, 'Name'));
    card.append(copyRow('Description', surface.description, 'Description'));
    // A loopback URL is appropriate for manual publication, not the OpenAI tunnel picker.
    ...
    if (url) {
      ...
      card.append(el('p', 'hint', surface.id === 'desktop' && !config.tunnel.desktopTunnelId
        ? 'Pick this connector’s own tunnel — paste its ID in step 2 first.' : 'Choose Tunnel, then pick this connector’s tunnel.'));
    }
    if (surface.detail && surface.state === 'error') card.append(el('p', 'hint is-warn', surface.detail));
    // Published is only our side of the connection. A current-endpoint request is separate evidence.
    if (surface.state === 'live') card.append(surface.lastRequestAt === null
      ? el('p', 'hint is-warn', 'Not created in ChatGPT yet — ChatGPT has never called this connector.')
      : el('p', 'hint', surface.lastToolCallAt === null
    ...
    if (surface.tools.length > 0) card.append(el('p', 'hint', `Tools: ${surface.tools.join(', ')}`));
    return card;
  });
}
/** Unknown health facts stay unknown rather than receiving plausible-looking defaults. */
function facts(next: AppState): HTMLElement[] {
  const { status, config } = next;
  const rows: [string, string, boolean?][] = [];
  const health = status.health;
  if (isRunning(status.state) && config.tunnel.kind === 'manual') {
    rows.push(['Local MCP server', status.localUrl ? 'ready' : 'starting…']);
    rows.push(['Remote connection', status.lastRequestAt === null ? 'not yet verified' : 'verified by an observed MCP client request', status.lastRequestAt === null]);
    if (status.localUrl) rows.push(['Local server', status.localUrl.replace(/^https?:\/\//, '')]);
  } else if (isRunning(status.state)) {
    rows.push([config.tunnel.kind === 'openai' ? 'Route to OpenAI' : 'Cloudflare route', health?.route ?? 'starting…']);
    rows.push(['Poll errors', health?.pollErrors === null || health?.pollErrors === undefined ? '—' : String(health.pollErrors), (health?.pollErrors ?? 0) > 0]);
    const probe = health?.probe ?? null;
    rows.push(['Tunnel → this app', probe ?? 'checking…', probe !== null && probe !== 'ok' && probe !== 'success' && probe !== 'healthy']);
    rows.push(['Tunnel uptime', duration(health?.uptimeSeconds ?? null)]);
    // A request without any tool call can indicate disabled Developer mode; surface that fact.
    if (status.lastRequestAt !== null) rows.push(['ChatGPT ran a tool', status.lastToolCallAt === null ? 'never — check Developer mode' : ago(status.lastToolCallAt), status.lastToolCallAt === null]);
    if (health?.clientVersion) rows.push(['Tunnel client', health.clientVersion]);
    if (status.localUrl) rows.push(['Local server', status.localUrl.replace(/^https?:\/\//, '')]);
  } else rows.push([config.tunnel.kind === 'openai' ? 'Route to OpenAI' : 'Transport', 'not running']);
  rows.push(['Tools ChatGPT can see', `${toolsOn(next)} available · ${config.roots.length} folder${config.roots.length === 1 ? '' : 's'}`]);
...
/** Repaint only aging numbers; a locally ready manual server must not claim a remote handshake. */
function paintClock(): void {
  ...
  $('liveNote').textContent = state.config.tunnel.kind === 'manual' && running
    ? status.lastRequestAt === null ? 'remote unverified' : `MCP request ${ago(status.lastRequestAt)}`
    : running ? status.handshakeAt === null ? 'no handshake yet' : `verified ${ago(status.handshakeAt)}` : '';
}
window.setInterval(paintClock, 1000);
function step(name: string): HTMLElement { return document.querySelector<HTMLElement>(`[data-step="${name}"]`)!; }
/** Build explicit text/strong nodes without interpreting markup. */
function frag(before: string, bold: string, after: string): DocumentFragment {
  const fragment = document.createDocumentFragment();
  fragment.append(before, el('strong', '', bold), after);
  return fragment;
}

// ------------------------------------------------------------------- log
/** Count retained model data, never scrape the already-rendered activity DOM for state. */
function paintProblems(): void {
  recentProblems = retainedProblemFlags.filter(Boolean).length;
  for (const id of ['homeProblems', 'logProblems']) {
    const badge = $(id);
    badge.hidden = recentProblems === 0;
    badge.textContent = `${recentProblems} problem${recentProblems === 1 ? '' : 's'}`;
  }
  if (state) renderHomeCockpit(state, { setupProblem: missingStep(state)?.text ?? null, problems: recentProblems });
}
...
/** Hidden feeds report zero geometry. Only real visible scrolling changes the remembered pin. */
const pinned = new Map<string, boolean>(FEEDS.map((id) => [id, true]));
function atBottom(view: HTMLElement): boolean { return view.scrollTop + view.clientHeight >= view.scrollHeight - 24; }
function stickToNewest(id: string): void {
  if (pinned.get(id) === false) return;
  const view = $(id);
  view.scrollTop = view.scrollHeight;
}
for (const id of FEEDS) $(id).addEventListener('scroll', () => {
  const view = $(id);
  if (view.clientHeight === 0) return;
  pinned.set(id, atBottom(view));
});
function addLogLine(entry: LogEntry): void {
  retainedProblemFlags.push(entry.level !== 'info');
  const removedProblem = retainedProblemFlags.length > 500 ? retainedProblemFlags.shift() === true : false;
  ...
  // Both newly added errors and errors retired by quiet traffic update the bounded count.
  if (entry.level !== 'info' || removedProblem) paintProblems();
}
$('logFilter').addEventListener('click', (event) => {
  const button = (event.target as HTMLElement).closest<HTMLButtonElement>('[data-filter]');
  if (!button) return;
  for (const other of $('logFilter').querySelectorAll('button')) other.classList.toggle('is-sel', other === button);
  $('fullFeed').classList.toggle('only-bad', button.dataset.filter === 'bad');
  stickToNewest('fullFeed');
});
/** Activity agent filtering is separate from the recorded-session timeline filter. */
let agentFilter: string | null = null;
function applyAgentFilter(): void {
  for (const row of $('fullFeed').querySelectorAll<HTMLElement>('p')) row.hidden = agentFilter !== null && (row.dataset.agent ?? null) !== agentFilter;
}
function paintAgentFilter(swarm: SwarmState): void {
  ...
  if (!swarm.running) {
    box.hidden = true;
    box.replaceChildren();
    if (agentFilter !== null) { agentFilter = null; applyAgentFilter(); }
    return;
  }
  ...
  if (agentFilter !== null && !swarm.agents.some((agent) => agent.id === agentFilter)) agentFilter = null;
  box.replaceChildren(...choices.map((choice) => {
    const button = document.createElement('button');
    button.type = 'button';
    button.textContent = choice.label;
    button.classList.toggle('is-sel', choice.id === agentFilter);
    button.addEventListener('click', () => { agentFilter = choice.id; paintAgentFilter(swarm); applyAgentFilter(); });
    return button;
  }));
  box.hidden = false;
  applyAgentFilter();
...
// --------------------------------------------------------------- wiring
async function addFolder(): Promise<void> { const next = await run(api.addRoot()); if (next) apply(next); }
async function toggleConnection(): Promise<void> {
  if (!state) return;
  // Exactly the same predicate as the button label, including offline retrying transports.
...
async function runChecks(): Promise<void> {
  ...
  try {
    ...
    $('checkList').replaceChildren(...result.checks.map((check) => {
      const li = el('li', check.status === 'pass' ? 'check is-ok' : check.status === 'fail' ? 'check is-bad' : `check is-${check.status}`);
      const mark = el('span', 'check-mark', check.status === 'pass' ? '✓' : check.status === 'fail' ? '!' : check.status === 'skipped' ? '–' : '…');
      const body = el('div');
  ...
  } finally { button.disabled = false; $('runChecksLabel').textContent = 'Run checks'; }
}
$('runChecks').addEventListener('click', () => void runChecks());
$('closeChecks').addEventListener('click', () => { $('checksBox').hidden = true; });
$('themeBtn').addEventListener('click', () => {
  if (!state) return;
  // Derive rapid toggles from requested state, not an older IPC acknowledgement.
...
$('wizExpand').addEventListener('click', () => { showAllSteps = !showAllSteps; if (state) apply(state); });
$('connectBtn').addEventListener('click', () => void toggleConnection());
$('wizConnect').addEventListener('click', () => void toggleConnection());
$('pickBinary').addEventListener('click', async () => { const next = await run(api.pickBinary()); if (next) apply(next); });
for (const id of ['copyLog', 'copyLogText']) $(id).addEventListener('click', async () => {
  const text = await run(api.getLogText());
  if (text === null) return;
  if (await run(api.writeClipboard(text))) toast('Activity copied');
});
$('copyLogJson').addEventListener('click', async () => {
  const text = await run(api.getLogJson());
  if (text === null) return;
  if (await run(api.writeClipboard(text))) toast('Activity JSON copied');
});
// Dedicated blur IPC prevents half-typed credentials travelling in a settings snapshot.
$('apiKey').addEventListener('blur', async () => {
  ...
  if (next) {
    // Never erase a newer value entered during safeStorage/IPC, nor discard a failed draft.
    if (input.value === submitted) input.value = '';
    apply(next);
    toast('API key stored');
  }
});
$('removeApiKey').addEventListener('click', async () => { const next = await run(api.setApiKey('')); if (next) { apply(next); toast('API key removed'); } });
for (const id of ['autoConnect', 'minimizeToTray', 'privacyScreenshots', 'tunnelKind', 'tunnelId', 'desktopTunnelId']) $(id).addEventListener('change', () => void save());
...
async function refresh(): Promise<void> { const next = await run(api.getState()); if (next) apply(next); }
buildGroups();
